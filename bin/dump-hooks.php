 ...
 * @usage: wp eval-file wp-content/plugins/the-camels-assistant/bin/dump-hooks.php
 *
...
foreach ( $runtime->_called_doc_hooks as $class_name => $hooks ) {
	...
	if ( 'TheCamels\\Assistant\\Runtime' === $class_name ) {
		$callback_object_name = '$this';
	} else {
		$property_name = array_search( $class_name, $objects );
		if ( ! $property_name ) {
			WP_CLI::warning( str_replace( 'TheCamels\\Assistant\\', '', $class_name ) . ' skipped, no instance available' );
			continue;
		}
	...
	WP_CLI::log( str_replace( 'TheCamels\\Assistant\\', '', $class_name ) . ' added ' . $count . ' hooks' );
}
